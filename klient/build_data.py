#!/usr/bin/env python3
"""УСТАРЕЛО — не запускать. Используйте ../rebuild.py.

В этом скрипте ошибка разбиения на рабочие дни: новый день начинается только при
сбросе номера визита (`n < prev` или `n == 1 and prev != 1`). Два подряд идущих дня
с одним визитом под номером 1 не разрываются, их записи сливаются, и шаг 2 оставляет
только первый `total`. Из-за этого терялось 32 визита и 336 050 ₸ выручки.
Файл оставлен для истории; data.js теперь собирает rebuild.py.
